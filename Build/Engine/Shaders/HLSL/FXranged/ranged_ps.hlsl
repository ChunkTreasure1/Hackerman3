...
//
	...
	float sinExp = sin(u_objectData.timeSinceCreation*3)*0.2;
	float cosAndSinExp = cos(sinExp*4)*0.1;

	//float alpha = u_albedo.Sample(u_pointSampler, Upanning).a;
	//
	float2 UV = input.texCoords;
	//float panningExp = 0.75;

	//float2 invertedUpanning = float2(UV.x-timeloop, UV.y+panningExp);

	//int timeFlooreddas = (floor(u_objectData.timeSinceCreation*2))/(floor(u_objectData.timeSinceCreation*2));

	//float vOffset = (timeFloored)*0.5;

	//float goldenExponent = (panningExp+vOffset);

	//float2 Upanning = float2(UV.x+u_objectData.timeSinceCreation, UV.y+goldenExponent);
	...
	//float opacityspeed = 2.5;
	float speed = 2;

	float timeFloored = floor(u_objectData.timeSinceCreation*(2*speed));
	float timeCappedTo1 = saturate(timeFloored); ///max(timeFloored, 1)); //anti division of 0 B)
	float timeOff = (timeCappedTo1)*0.5;
	

	
	//float timeFloored2 = round(frac(-1+u_objectData.timeSinceCreation*0.5));
	//float timeCappedTo2 = saturate(timeFloored2); //anti division of 0 B)
	//float timeOff2 =(timeCappedTo2)*0.5;

	
	float uvOff = 0.75;


	float2 offsetUV = float2 (UV.x+u_objectData.timeSinceCreation*(1*speed), UV.y+(uvOff+timeOff));

	///


	float3 col = u_albedo.Sample(u_linearPointSampler, offsetUV).rgb;
	float alpha = u_albedo.Sample(u_linearPointSampler, offsetUV).a;

	float op = alpha*(pow(vCol, 2));
