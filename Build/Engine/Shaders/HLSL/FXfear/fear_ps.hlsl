...
//
	...
	float speed = 0.5;
	;
	float timeloop = frac(u_objectData.timeSinceCreation*speed);

	float UVswitch = saturate(floor(u_objectData.timeSinceCreation*(speed*2)));
	float newUVoffset = 0.5*UVswitch;
	...
	float2 finalUVoffset = float2 (UV.x*0.25+newUVoffset, UV.y+1-timeloop);

	

	//float2 offsetUV = float2(UV.x*0.25+newUVoffset, UV.y*0.25);

	//float2 Vpanning = float2(0.25+newUVoffset, UV.y+1-timeloop);


	//float4 albedo = u_albedo.Sample(u_pointSampler, Upanning);
	float3 col = u_albedo.Sample(u_linearPointSampler, finalUVoffset).rgb;
	float alpha = u_albedo.Sample(u_linearPointSampler, finalUVoffset).a;

	float op = float(alpha*vCol);
	//float opPow = pow(op, 2);
