...
//
	...
	float2 Upanning = float2(UV.x+timeloop, UV.y);


	//float4 albedo = u_albedo.Sample(u_pointSampler, Upanning);
	float3 col = u_albedo.Sample(u_linearPointSampler, 1-Upanning).rgb;
	float alpha = u_albedo.Sample(u_linearPointSampler, Upanning).a;
