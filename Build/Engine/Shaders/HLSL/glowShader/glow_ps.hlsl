...
//
	...
	//sineCos
	float sineTime = sin(u_objectData.timeSinceCreation*6);
	float sineTimeForLumin = ((u_objectData.timeSinceCreation*2)/2)(sin(u_objectData.timeSinceCreation*2));
	//float cosAndSinExp = cos(sineTime*4)*0.1;

	//float4 albedo = u_albedo.Sample(u_pointSampler, Upanning);
	float3 colRedGreen = float3 (0.7, 0.0, 0.1);
	float3 colRedBlue = float3 (0.7, 0.0, 0.2);

	float3 colShift = lerp(float3 (colRedBlue), float3 (colRedGreen), sineTime);

	float lumin = lerp(1.0, 4.0, sineTimeForLumin);

	Output output;
	output.color = float4 ((colShift),1);
	output.luminance = lumin;
	output.id = 0;
