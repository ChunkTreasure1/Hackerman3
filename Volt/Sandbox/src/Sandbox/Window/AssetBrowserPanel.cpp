...
void AssetBrowserPanel::UpdateMainContent()
{
	...
	if (EditorUtils::MeshImportModal("Import Mesh##assetBrowser", myMeshImportData, myMeshToImport.path))
	{
		Reload();
	}
	...
	if (x > myViewBounds[0].x && y > myViewBounds[0].y && x < myViewBounds[1].x && y < myViewBounds[1].y)
	{
		for (const auto& path : e.GetPaths())
		{
			if (!std::filesystem::is_directory(path))
			{
				...
				FileSystem::Copy(path, myCurrentDirectory->path / (tempName + path.extension().string()));
			}
		}
...
Ref<DirectoryData> AssetBrowserPanel::ProcessDirectory(const std::filesystem::path& path, Ref<DirectoryData> parent)
{
	...
	for (const auto& entry : std::filesystem::directory_iterator(path))
	{
		if (!entry.is_directory())
		{
			...
			if (assetData.type != Volt::AssetType::None)
			{
	...
	ImGui::BeginChild("##controlsBar", { 0.f, height });
	{
		...
		UI::ShiftCursor(5.f, 4.f);
		{
			...
			for (size_t i = 0; i < myDirectoryButtons.size(); i++)
			{
				...
				if (ImGui::BeginChild(dirName.c_str(), { buttonWidth, height - 4.f }))
				{
...
void AssetBrowserPanel::RenderDirectories(std::vector<Ref<DirectoryData>>& dirData)
{
	for (auto& dir : dirData)
	{
		...
		const ImVec2 itemSize = { myThumbnailSize + itemPadding, myThumbnailSize + itemHeightModifier + itemPadding };
		const ImVec2 minChild = cursorPos + windowPos - ImVec2{ 0.f, scrollYOffset };
		const ImVec2 maxChild = minChild + itemSize;

		const bool itemHovered = ImGui::IsMouseHoveringRect(minChild, maxChild);

		ImVec4 childBgCol = ImVec4{ 0.28f, 0.28f, 0.28f, 1.f };

		if (itemHovered && ImGui::IsMouseDown(ImGuiMouseButton_Left))
		{
			childBgCol = ImVec4{ 0.3f, 0.6f, 1.f, 1.f };
		}
		else if (itemHovered)
		{
			childBgCol = ImVec4{ 0.2f, 0.56f, 1.f, 1.f };
		}
		else if (dir->selected)
		{
			childBgCol = ImVec4{ 0.f, 0.44f, 1.f, 1.f };
		}
...
void AssetBrowserPanel::RenderAssets(std::vector<AssetData>& assetData)
{
	for (AssetData& asset : assetData)
	{
		...
		Ref<Volt::Image2D> icon = (myAssetPreviews.find(asset.handle) != myAssetPreviews.end() && myAssetPreviews.at(asset.handle)->IsRendered()) ? myAssetPreviews.at(asset.handle)->GetPreview() : nullptr;
...
bool AssetBrowserPanel::RenderFilePopup(AssetData& data)
{
	...
	if (UI::BeginPopup(data.path.string(), ImGuiPopupFlags_MouseButtonRight))
	{
		DeselectAllAssets(myCurrentDirectory);
		DeselectAllDirectories(myCurrentDirectory);
		data.selected = true;
	...
	if (ImGui::MenuItem("Reimport"))
	{
		...
		if (FileSystem::Exists(fbxPath))
		{
			...
			if (currentMesh)
			{
				...
				if (Volt::MeshCompiler::TryCompile(Volt::AssetManager::GetAsset<Volt::Mesh>(fbxPath), data.path, material ? material->handle : Volt::Asset::Null()))
				{
