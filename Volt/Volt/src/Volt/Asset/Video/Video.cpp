...
#define CALC_FFMPEG_VERSION(a,b,c) ( a<<16 | b<<8 | c )
	...
	void Video::Update(float deltaTime)
	{
		...
		const float fps = (float)Utility::GetFPS(myReaderState.formatContext, myReaderState.videoStreamIndex);
	...
	int32_t Video::ReadNextFrame()
	{
		...
		constexpr int32_t maxSafeValue = 10000;
	...
	bool Video::GetFrameData(uint32_t*& buffer)
	{
		...
		int32_t result = sws_scale(myReaderState.swsContext, myReaderState.avFrame->data, myReaderState.avFrame->linesize, 0, myReaderState.decoderContext->height, myReaderState.avFrameBGR->data, myReaderState.avFrame->linesize);
		...
		if (result > 0)
		{
			...
			for (int32_t i = 0; i < myReaderState.decoderContext->height; i++)
			{
				for (int32_t j = 0; j < myReaderState.decoderContext->width; j++)
				{
					...
					buffer[i * myReaderState.width + j] = uint32_t(data[rgbIndex + 3]) << 24 |
						(data[rgbIndex + 2]) << 16 |
						(data[rgbIndex + 1]) << 8 |
						(data[rgbIndex]);
