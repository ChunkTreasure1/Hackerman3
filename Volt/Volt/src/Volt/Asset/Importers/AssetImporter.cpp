...
namespace Volt
{
	...
	bool ShaderImporter::Load(const std::filesystem::path& path, Ref<Asset>& asset) const
	{
		...
		{
			VT_CORE_ERROR("No shaders defined in shader definition {0}!", path.string().c_str());
			asset->SetFlag(AssetFlag::Invalid, true);
			return false;
		}
		...
		for (const auto input : inputTexturesNode)
		{
			...
			std::string name;
			VT_DESERIALIZE_PROPERTY(name, name, input, std::string("Null"));
		...
		// Make sure all textures defined in definition actually exist
		{
			...
			for (const auto& [binding, name] : inputTextures)
			{
				if (binding >= Shader::MinCustomTextureSlot() || binding <= Shader::MaxCustomTextureSlot())
				{
					...
					{
						VT_CORE_ERROR("Shader {0} does not have a texture input with binding {1}, but this is defined in definition!", path.string().c_str(), binding);
					}
					else
					{
	...
	bool MaterialImporter::Load(const std::filesystem::path& path, Ref<Asset>& asset) const
	{
		...
		VT_DESERIALIZE_PROPERTY(name, nameString, materialNode, std::string("Null"));

		std::unordered_map<uint32_t, Ref<SubMaterial>> materials;

		YAML::Node materialsNode = materialNode["materials"];
	...
	void VideoImporter::Save(const Ref<Asset>& asset) const
	{}
}