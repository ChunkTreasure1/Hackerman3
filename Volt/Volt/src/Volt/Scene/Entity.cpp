...
namespace Volt
{
	...
	void Entity::SetWorldPosition(const gem::vec3& position)
	{
		const gem::vec3 localPosition = GetPosition();
		const gem::vec3 worldPosition = GetWorldPosition();

		const gem::vec3 parentPos = worldPosition - localPosition;
		const gem::vec3 targetPos = position - parentPos;
		SetPosition(position);
	}

	void Entity::SetPosition(const gem::vec3& position)
	{
		auto& transComp = myScene->GetRegistry().GetComponent<TransformComponent>(myId).position = position;
	...
	{
		auto& transComp = myScene->GetRegistry().GetComponent<TransformComponent>(myId).rotation = rotation;
		if (myScene->GetRegistry().HasComponent<RigidbodyComponent>(myId))
		{
			auto actor = Physics::GetScene()->GetActor(*this);
			if (actor)
	...
	{
		auto& transComp = myScene->GetRegistry().GetComponent<TransformComponent>(myId).scale = scale;
	}
