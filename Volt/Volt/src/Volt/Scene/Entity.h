...
namespace Volt
{
	...
	public:
		...
		template<typename T>
		T* GetScript(const std::string& scriptName);
		...
		void SetWorldPosition(const gem::vec3& position);
	...
	template<typename T>
	inline T* Entity::GetScript(const std::string& scriptName)
	{
		const auto guid = ScriptRegistry::GetGUIDFromName(scriptName);
