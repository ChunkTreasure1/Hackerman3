...
namespace Volt
{
	...
	void PhysicsScene::Simulate(float timeStep)
	{
		...
		{
			myEntityScene->GetRegistry().ForEach<ScriptComponent>([this](Wire::EntityId id, ScriptComponent& scriptComp) // This is not actually "fixed"
				{
	...
	Ref<PhysicsActor> PhysicsScene::CreateActor(Entity entity)
	{
		...
		myPhysXScene->addActor(*actor->myRigidActor);
		return actor;
	}
	...
	void PhysicsScene::RemoveActor(Ref<PhysicsActor> actor)
	{
		...
		myDestructionQueue.emplace_back(actor);
	...
	void PhysicsScene::RemoveActors()
	{
		for (const auto& actor : myDestructionQueue)
		{
			for (auto& collider : actor->myColliders)
			{
				collider->DetachFromActor(actor->myRigidActor);
				collider->Release();
			}

			myPhysXScene->removeActor(*actor->myRigidActor);
			actor->myRigidActor->release();
			actor->myRigidActor = nullptr;
		}

		myDestructionQueue.clear();
	}
