...
namespace Volt 
{
	...
	void PhysicsErrorCallback::reportError(physx::PxErrorCode::Enum code, const char* message, const char* file, int line)
	{
		...
		switch (code)
		{
			...
			case physx::PxErrorCode::eDEBUG_INFO:
				VT_INFO("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			...
			case physx::PxErrorCode::eINTERNAL_ERROR:
				VT_ERROR("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
	...
	{
		VT_ERROR("[PhysX Error]: {0}:{1} - {2}", file, line, exp);
	}
}