...
namespace Volt
{
	Volt::MeshInfo NavMeshGenerator::GetNavMeshVertices(const Volt::Entity aEntity, const float& aAngle)
	{
		...
		if (meshComp.walkable)
		{
			...
			{
				v.position *= transformComp.scale;
				v.position += transformComp.position;
			}
	...
	std::vector<Volt::NavMeshCell> NavMeshGenerator::GenerateNavMeshCells(const Volt::NavMesh& aNavMesh)
	{
		std::vector<Volt::NavMeshCell> result;

		for (uint32_t i = 0; i < aNavMesh.Indices.size(); i += 3)
		{
			// #SAMUEL_TODO: Set neighbour cells
