...
namespace Volt
{
	...
	void Image2D::Invalidate(const void* aData)
	{
		...
		if (!Utility::IsDepthFormat(mySpecification.format) || isTypeless)
		{
			...
			if (shaderDesc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE2DARRAY)
			{
				shaderDesc.Texture2DArray.ArraySize = mySpecification.layers;
				shaderDesc.Texture2DArray.MipLevels = mySpecification.mips;
