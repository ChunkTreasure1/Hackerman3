	...
	bool ShaderCompiler::TryCompile(const std::vector<std::filesystem::path>& aPaths, std::unordered_map<ShaderStage, ComPtr<ID3D10Blob>>& outCode)
	{
		...
		for (const auto& path : aPaths)
		{
			...
			// Compile shader
			{
				...
				HRESULT result = D3DCompileFromFile(path.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", Utility::GetProfileFromStage(stage).c_str(), compileFlags, 0, outCode[stage].GetAddressOf(), &error);
				if (error)
				{
