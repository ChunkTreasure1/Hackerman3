...
namespace Volt
{
	...
	void Shader::Bind() const
	{
		...
		if (myShaders.find(ShaderStage::Compute) == myShaders.end())
		{
	...
	void Shader::ReflectStage(ShaderStage aStage, ComPtr<ID3D10Blob> aBlob)
	{
				...
				for (uint32_t j = 0; j < desc.Variables; j++)
				{
					...
					switch (param.type)
					{
						case ElementType::Bool:
							*(bool*)&materialBuffer.data[param.offset] = false;
							break;

						case ElementType::Float:
							*(float*)&materialBuffer.data[param.offset] = 0.f;
							break;

						case ElementType::Float2:
							*(gem::vec2*)&materialBuffer.data[param.offset] = 1.f;
							break;

						case ElementType::Float3:
							*(gem::vec3*)&materialBuffer.data[param.offset] = 1.f;
							break;

						case ElementType::Float4:
							*(gem::vec4*)&materialBuffer.data[param.offset] = 1.f;
							break;

						case ElementType::Int:
							*(int32_t*)&materialBuffer.data[param.offset] = 0;
							break;

						case ElementType::UInt:
							*(uint32_t*)&materialBuffer.data[param.offset] = 0;
							break;

						case ElementType::UInt2:
							*(gem::vec2ui*)&materialBuffer.data[param.offset] = 1;
							break;

						case ElementType::UInt3:
							*(gem::vec3ui*)&materialBuffer.data[param.offset] = 1;
							break;

						case ElementType::UInt4:
							*(gem::vec4ui*)&materialBuffer.data[param.offset] = 1;
							break;
					}
				}
			}
		...
		if (aStage == ShaderStage::Vertex)
		{
			...
			VT_CORE_INFO("		Inputs: {0}", inputs.size());
			for (const auto& input : inputs)
			{
				VT_CORE_INFO("			{0}: {1}, {2} bytes", input.SemanticName, input.SemanticIndex, input.AlignedByteOffset);
			}
		}
	...
	std::vector<D3D11_INPUT_ELEMENT_DESC> Shader::ReflectInputDescription(ID3D11ShaderReflection* aReflector)
	{
		...
		for (uint32_t i = 0; i < shaderDesc.InputParameters; i++)
		{
			...
			const bool isInstanceData = semanticName.find("_INSTANCE") != std::string::npos;
			if (isInstanceData)
			{
				slot = 1;
			}
